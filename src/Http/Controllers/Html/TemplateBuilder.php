...
namespace wh1110000\CmsL8\Http\Controllers\Html;
...
class TemplateBuilder extends HtmlBuilder {
	...
	public function googleTagManager($position = 'body'){
		...
		$googleTagManagerId = app('SettingsManager')->get('GOOGLE_TAG_ID');
		...
		if($googleTagManagerId){
		...
		} elseif($googleTagManagerCode = app('SettingsManager')->get('GOOGLE_CODE')){
			...
			if(App::environment(['production'])) {
				...
				$html .=  '<!--'.(is_null($googleTagManagerCode) ? 'Google Tag Manager Code Not Provided' : ': ').'-->';
			}
	...
	public function favicons() {
		...
		if($favicon = app('SettingsManager')->get('FAVICON')){
