...
class Fields {
	...
	public function isRequired(){
		...
		$request = optional(request()->route()->controller);


		if(isset($request->repository->requestClass)){

			$request = $request->repository->requestClass;

			$rules = $request->getRules();

			if(isset($rules[$this->name])){

				foreach($rules[$this->name] as $rule){
	...
	public function add($name = null, $attributes = []){
		...
		if(optional(\Form::getModel())->hasMultilanguage()){
				...
				switch ( $this->field ) {
					...
					case 'select' :
						...
						if(\Form::getModel()){
							...
							$this->selected(\Form::getModel()->{$this->name});
						}
