...
//if (!function_exists('app('SettingsManager')->get')) {
...
//if(!function_exists('getActiveLanguages')){
	...
	function getActiveLanguages($exclude = [], $translationBy = null){


		$locales = collect();//app('activeLocales');

		if($exclude && 1==2){

			$locales = $locales->whereNotIn('code', $exclude);
		}
...
//if(!function_exists('getCurrentLanguage')){
	...
	function getCurrentLanguage(){
		...
		return 'uk';
		//if(\Workhouse\Core\Services\Route::isAdminRoute()){
		...
		if(!request()->urlLocale){

			return \Illuminate\Support\Str::before(\Illuminate\Support\Str::finish(request()->path(), '/'), '/');
		}
	...
	function getCurrentUrlLocale(){
		...
		if(config('app.translationby') == 'country'){
			...
			$locale = request()->urlLocale ?? \Illuminate\Support\Str::before(\Illuminate\Support\Str::finish(request()->path(), '/'), '/');
		}
...
//if(!function_exists('isLanguage')){
	...
	function isLocale($locale){
		...
		return getCurrentUrlLocale() == $locale;
	}
//}
